...
describe("/api/users - Endpoint", () => {
    ...
    describe('When a user is logged in they can create a post', () => {
      test("Returns the correct status code", async () => {
        let response = await supertest(app)
          .post("/api/posts/new")
          .set('Cookie', `jwt=${token}`)
          .send({ image: 'Another', imageCloudId: 'TestId', caption: 'Caption for post', user: postUser._id });
  ...
  describe('Get post - GET /:id endpoint', () => {
    test('The correct status and response code are returned when getting a single post', async () => {
        ...
        expect(finalResponse.body.imageCloudId).toBe('testId');
  ...
  describe('Get all posts - GET /all endpoint', () => {
    test('Should return the correct response status', async () => {
      ...
      .post("/api/posts/new")
      .set('Cookie', `jwt=${token}`)
      .send({ image: 'post 1', imageCloudId: 'testId', caption: 'Caption for post 1', user: postUser._id });
      const response = await supertest(app)
      .get('/api/posts/all')
      .set('Cookie', `jwt=${token}`)
    ...
    test('Should return an array of all created posts from database', async () => {
      ...
      .post("/api/posts/new")
      .set('Cookie', `jwt=${token}`)
      .send({ image: 'post 2', imageCloudId: 'testId', caption: 'Caption for post 2', user: postUser._id });
      await supertest(app)
      .post("/api/posts/new")
      .set('Cookie', `jwt=${token}`)
      .send({ image: 'post 3', imageCloudId: 'testId', caption: 'Caption for post 3', user: postUser._id });
      const response = await supertest(app)
      .get('/api/posts/all')
      .set('Cookie', `jwt=${token}`)
  ...
  //UPDATE:
  describe('Updating a post endpoints', () => {
    describe('/addLike', () => {
      test('Adds a like to the likedBy array if the user has not yet liked a post', async () => {
        await supertest(app)
        .post("/api/posts/new")
        .set('Cookie', `jwt=${token}`)
        .send({ image: 'post 1', imageCloudId: 'testId', caption: 'Caption for post 1', user: postUser._id });
      ...
      test('Cannot ADD A LIKE to a post when a user is not logged in', async () => {
        let response = await supertest(app)
          .put(`/api/posts/addLike`)
        expect(response.body.message).toBe('Unauthorized without a token');
        expect(response.statusCode).toBe(401);
...
// The id is now passed in REQ body - update to show this:
